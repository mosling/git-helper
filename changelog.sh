...
OUTFOLDER="${HOME}/development/changelog"
CLEANUP="no"
OVERLAP=3
...
if [ $# -eq 0 ]; then
	...
	TMPLPATH=$(pwd | rev | cut -d '/' -f1,2 | rev)
	if [ -f ".gitproject" ]; then
		TMPLPRJ=$(cat .gitproject)
	else
		TMPLPRJ=$(echo $TMPLPATH | cut -d'/' -f1)
	fi
	TMPLREPO=$(echo $TMPLPATH | cut -d'/' -f2)
	TMPLROWHL="Merge branch"
	TMPLGITURL=$(git remote get-url origin | sed -e "s/^[^@]*@//" -e "s/:.*//")
	TMPLGIT=github
	if [[ $TMPLGITURL == *"stash"* ]]; then
		TMPLGIT=bitbucket
		TMPLGITURL=intranet.demandware.com
	fi

	TMPLGIT=$(readValue "Git Repository Type                           : " "$TMPLGIT")
	TMPLGITURL=$(readValue "Git Repository Address                        : " "$TMPLGITURL")
	TMPLPRJ=$(readValue "Project Name (contains repositories)          : " "$TMPLPRJ")
